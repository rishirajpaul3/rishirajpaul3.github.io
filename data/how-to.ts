...
export const guides: Guide[] = [
  {
    ...
    title: "How to set up cold email deliverability properly",
    category: "Outbound",
    readTime: "9 min read",
    date: "May 2026",
    desc: "Most cold email problems are not writing problems. They are infrastructure problems. If your emails are landing in spam, the copy is the last thing you should be fixing.",
    tools: ["Instantly", "Smartlead", "Google Domains", "MXToolbox", "Mail-Tester"],
    sections: [
      {
        heading: "Why deliverability breaks before you ever send",
        body: `Cold email has a reputation problem, and most of it is deserved. Inboxes are flooded with badly written, poorly targeted messages from domains that were never set up correctly. The spam filters are good now. They catch things that would have sailed through five years ago.
...
The result is that even well-written, well-targeted emails frequently land in spam not because of the copy but because of the infrastructure sending them. Before you spend an hour perfecting your opener, spend two hours getting the technical setup right. It compounds in a way that good copy never does.`,
      ...
      {
        heading: "Start with the right domain setup",
        body: `Never send cold email from your primary company domain. This is the most common mistake and the most costly one to fix once it goes wrong. Your main domain carries the reputation of every email your company has ever sent. One bad cold campaign can damage it for months.
...
Use MXToolbox to verify all three are live and correctly configured before you send a single email. It is free and takes two minutes.`,
      ...
      {
        heading: "Warming up is not optional",
        body: `A fresh domain has zero reputation. Sending 100 cold emails from a brand new domain on day one is the fastest way to get flagged. Mail servers treat volume spikes on new domains as a strong signal of spam.
...
During warmup, keep your manual sending volume at 10 to 15 emails per day per account. After warmup completes, you can push to 30 to 40 per day safely. Going beyond 50 per day per account requires more established sending history.`,
      ...
      {
        heading: "Set up multiple sending accounts from the start",
        body: `Do not run all your cold outreach through a single account. Set up three to five sending accounts from the beginning. This does two things. It spreads your sending volume so no single account accumulates enough activity to trigger throttling. And it gives you redundancy if one account gets flagged or restricted.
...
One practical consideration: make sure each sending account has a real profile. A profile photo, a filled-out bio, a few weeks of regular email activity. Accounts that look like they were created purely for outreach get flagged faster.`,
      ...
      {
        heading: "Test before every campaign launch",
        body: `Before you activate any campaign, run your email through a deliverability tester. Mail-Tester is the standard tool. Send an email to the address it gives you, then check your score. Anything above 9 out of 10 is fine to send.
...
GlockApps is a more detailed alternative that shows you placement across different email providers. Worth using before a high-volume campaign launch.`,
      ...
      {
        heading: "Monitor bounce rates obsessively",
        body: `A bounce rate above 2 percent is a serious problem. Hard bounces — emails sent to addresses that do not exist — are the most damaging because they signal to receiving servers that your list is dirty.
...
Getting the infrastructure right does not guarantee results. But getting it wrong guarantees the opposite.`,
  ...
  {
    slug: "first-n8n-workflow-with-ai",
    title: "How to build your first n8n workflow with AI",
    category: "n8n",
    readTime: "8 min read",
    date: "May 2026",
    desc: "n8n looks complicated until you understand the mental model. Once that clicks, you can wire AI into almost any process in under an hour. Here is where to start.",
    tools: ["n8n", "OpenAI", "Webhook", "Google Sheets"],
    sections: [
      {
        heading: "The mental model that makes everything click",
        body: `n8n is a visual automation tool. You build workflows by connecting nodes, where each node does one specific thing and passes its output to the next. A node might receive a webhook, call an API, transform data, or send a message. The workflow is just a pipeline of these steps.
...
Start with one workflow that does one thing. Build it, run it, understand why it works. Everything else is a variation of the same pattern.`,
      ...
      {
        heading: "Getting n8n running",
        body: `You have two options. n8n Cloud is the fastest way to start. Sign up, get a URL, and you are building workflows in five minutes. It handles hosting, updates, and scaling for you. The free tier gives you a limited number of workflow executions per month, which is plenty to learn.
...
Once you are logged in, the interface shows you a canvas. Nodes sit on the canvas and connect to each other with lines that represent the data flow. The left panel is your node library. That is the full extent of the UI you need to understand right now.`,
      ...
      {
        heading: "Your first workflow: classify an email reply with AI",
        body: `Email reply classification is the ideal first workflow. It is genuinely useful, straightforward to build, and teaches you the core pattern you will use in every workflow after it.
...
If it works, you have built your first AI workflow. If it errors, the execution log shows you exactly which node failed and what the error was. Fix it there.`,
      ...
      {
        heading: "Working with data in n8n",
        body: `Everything in n8n is structured as an array of items. Every node receives items and outputs items. An item is a JSON object with a json property that holds the actual data.
...
If you are unsure what a field is called, click the expression editor (the small icon next to any input field). It shows you a tree of all available data you can reference.`,
      ...
      {
        heading: "Adding error handling once it runs",
        body: `The most common mistake in n8n workflows is shipping them without any error handling and discovering the failure mode in production when something goes wrong at 2am.
...
The function node is your escape hatch when built-in nodes cannot do exactly what you need. It lets you write JavaScript directly. It is also where most beginners get stuck. If you hit a wall with a function node, paste the node code and the input data into Claude Code and ask it to fix it. This saves significant debugging time.`,
  ...
  {
    slug: "claude-code-prospect-research",
    title: "How I use Claude Code to research prospects in minutes",
    category: "Claude Code",
    readTime: "7 min read",
    date: "May 2026",
    desc: "The way I do account research has completely changed since I started treating Claude Code as a live research assistant. This is the exact workflow I run before any important outreach.",
    tools: ["Claude Code", "LinkedIn", "Clay", "Browser"],
    sections: [
      {
        heading: "What changed when I stopped Googling",
        body: `Before Claude Code, proper account research took 20 to 30 minutes per company. LinkedIn for the contact, their website for the positioning, Crunchbase for funding, job boards to understand what they are hiring for, recent news to find an angle. Good research. Slow research.
...
Claude Code collapses this. It can browse the web, read pages, and synthesise information in one pass. More importantly, it reasons about the information rather than just retrieving it. Ask it the right question and it tells you the angle, not just the facts.`,
      ...
      {
        heading: "The exact prompt structure I use",
        body: `The prompt matters more than anything else in this workflow. A bad prompt gets you a summary of the company's About page. A good prompt gets you a specific, actionable outreach brief.
...
I include any signals I already have. If I know they recently hired three SDRs, I include that. If their job posts mention they are switching CRMs, I include that. Claude Code incorporates the signals into its reasoning and produces more specific output.`,
      },
      {
        ...
        body: `The output Claude Code produces is a starting point, not a finished brief. Read it critically. It will sometimes miss context that is obvious to you as someone who understands the company's industry or situation. It will occasionally make a logical leap that does not hold up.
...
One pattern worth building: after a few weeks of using this workflow, you will notice that certain angles come up repeatedly for companies at similar stages or in similar industries. Document those patterns. They become your playbook for researching at scale.`,
      ...
      {
        heading: "Scaling it with Clay",
        body: `For one-off research before an important call, the Claude Code workflow works perfectly. For researching 50 accounts before a campaign, you need Clay.
...
The quality of the AI column output in Clay is not as high as a focused Claude Code session on a single account. But it is dramatically faster for list-level research. Use Claude Code for your top 10 accounts. Use Clay for the rest.`,
      ...
      {
        heading: "When research is not worth doing",
        body: `Deep research only makes sense when the outreach warrants it. A personalised cold email to a senior decision maker at a target account deserves 5 minutes of research. A broad awareness campaign to 500 mid-market companies does not.
...
The mistake is treating research as always necessary. It is not. Relevance matters more than personalisation. A well-targeted email with a relevant offer to the right person beats a deeply personalised email to the wrong one every time.`,
  ...
  {
    slug: "clay-enrichment-table",
    title: "How to build a Clay enrichment table with AI columns",
    category: "Clay",
    readTime: "10 min read",
    date: "May 2026",
    desc: "Clay is the most powerful enrichment tool available right now. Most people use about 20 percent of what it can do. Here is how to build a table that takes a domain list and produces a campaign-ready contact list.",
    tools: ["Clay", "Apollo", "Hunter.io", "OpenAI", "HeyReach"],
    sections: [
      {
        heading: "How Clay actually works",
        body: `Clay is a spreadsheet where every column can be an API call, an enrichment lookup, or an AI operation that runs on every row. That is the entire product. Understanding that model changes how you design tables.
...
The power comes from chaining. The output of one column feeds into the prompt of the next. By the time a row reaches your outreach tool, it has passed through a dozen data sources and AI operations that have enriched, scored, and personalised it automatically.`,
      ...
      {
        heading: "Starting with the right input",
        body: `Every Clay table starts with a list. That list is usually one of three things: a CSV of company domains you want to target, a CSV of contacts with names and emails, or a live source like a Clay search that pulls accounts matching your ICP criteria.
...
Always add an email verification column at the end of the contact-finding chain. NeverBounce and ZeroBounce both have Clay integrations. Any contact with a low verification confidence score should be excluded from your campaign. Sending to unverified emails damages your deliverability.`,
      ...
      {
        heading: "The AI layer: scoring and personalisation",
        body: `Once your enrichment columns are populated, add the AI layer. This is where Clay becomes genuinely different from any other enrichment tool.
...
The quality of AI column output is directly proportional to the specificity of your prompt and the richness of the data in your other columns. A sparse table produces generic AI output. A well-enriched table produces usable personalisation.`,
      ...
      {
        heading: "Pushing to outreach and keeping it clean",
        body: `Once your table is enriched and scored, push the verified contacts directly to HeyReach for LinkedIn outreach or to Instantly and Smartlead for email. Clay has native integrations with both.
...
One thing worth building into every table: a status column that tracks what happened to each row. Did it get pushed? Did the email bounce? Did it convert? Keeping this in Clay alongside the enrichment data means you can analyse which enrichment signals correlate with outcomes over time. That analysis makes every subsequent table better.`,
  ...
  {
    slug: "linkedin-outreach-without-getting-flagged",
    title: "How to run LinkedIn outreach without getting your account flagged",
    category: "Outbound",
    readTime: "8 min read",
    date: "May 2026",
    desc: "LinkedIn throttles and restricts accounts that behave like bots. The fix is not sending less — it is sending smarter with the right infrastructure from day one.",
    tools: ["HeyReach", "LinkedIn Sales Navigator", "Clay"],
    sections: [
      {
        heading: "Why LinkedIn keeps restricting accounts",
        body: `LinkedIn's algorithm is genuinely good at detecting automated behaviour. It watches for patterns that humans do not exhibit naturally: sending connection requests at consistent intervals throughout the day, messaging immediately after connecting, activity at the same hours every day, high volume with no variation.
...
Account restrictions are not random. They follow a clear pattern: too many connection requests in a short window, too many messages sent to unconnected profiles, or a spike in activity that does not match the account's historical behaviour. Understanding what triggers restrictions tells you exactly what to avoid.`,
      ...
      {
        heading: "The infrastructure that actually protects you",
        body: `The most important structural decision is using multiple LinkedIn accounts rather than one. HeyReach is built specifically for this. It connects several LinkedIn profiles (typically belonging to different team members or dedicated outreach accounts) and distributes your sending volume across all of them.
...
Vary the timing. Humans do not send exactly 15 connection requests at exactly the same time every day. Good automation tools mimic natural variation. HeyReach adds random delays between actions and distributes activity across the working hours you specify. Turn these features on.`,
      ...
      {
        heading: "Writing connection requests that get accepted",
        body: `Your acceptance rate tells you a lot. Below 20 percent means your targeting or your messaging needs work. Above 35 percent means you are reaching the right people with a message that feels personal.
...
Do not include a pitch or a link in the connection request. This is the fastest way to get reported as spam. The only goal of the connection request is to be accepted.`,
      ...
      {
        heading: "Sequencing after they connect",
        body: `Wait 24 to 48 hours after someone accepts your connection before sending a message. Sending immediately reads as automated even when it is not. The delay also gives you a moment to check their recent activity before you message them, which can add a specific hook to your opener.
...
Never send the same message sequence to everyone. Even with personalisation variables, if enough people report the same template as spam, LinkedIn pattern-matches it and flags accounts running it.`,
      ...
      {
        heading: "Monitoring and fixing problems early",
        body: `Check your acceptance rate per account weekly. If any account drops below 20 percent for two consecutive weeks, pause it and review the messaging. A declining acceptance rate is LinkedIn's early warning before a restriction.
...
The accounts that run the longest are the ones that never spike. Consistent, moderate, human-like activity over weeks and months builds the kind of account history that LinkedIn trusts.`,
  ...
  {
    slug: "hubspot-n8n-integration",
    title: "How to connect HubSpot to anything using n8n",
    category: "n8n",
    readTime: "9 min read",
    date: "May 2026",
    desc: "HubSpot's native integrations cover the popular tools. n8n connects it to everything else — without writing a custom integration from scratch.",
    tools: ["n8n", "HubSpot", "Webhook", "REST API"],
    sections: [
      {
        heading: "What n8n gives you that HubSpot's integrations do not",
        body: `HubSpot has a marketplace with hundreds of integrations. For the tools that are in it, the native integrations are usually fine. For anything else — a custom data source, a niche tool, a homegrown system — you are writing a Zapier zap, paying for a middleware service, or building a custom integration yourself.
...
The most common use cases I see are: syncing data between HubSpot and a tool that does not have a native integration, automating multi-step processes that happen when a HubSpot property changes, and enriching HubSpot records with data from external sources on a trigger.`,
      ...
      {
        heading: "Setting up HubSpot credentials in n8n",
        body: `Open n8n, go to Settings, and click Credentials. Create a new credential and search for HubSpot API. You have two authentication options: private app token (recommended) or OAuth.
...
One important note: private app tokens do not expire, but they can be revoked. If a workflow stops working suddenly, check that the private app is still active in HubSpot before debugging the workflow itself.`,
      ...
      {
        heading: "The most useful HubSpot node operations",
        body: `The native HubSpot node in n8n covers contacts, companies, deals, and engagements. Within those objects, the operations you will use most often are create or update record, get record, search records, and add note.
...
Add note creates an engagement on a contact or deal record. This is how you log workflow activity back into HubSpot so the sales team can see what automation has done to a record without leaving the CRM.`,
      ...
      {
        heading: "Using HubSpot webhooks as triggers",
        body: `HubSpot can send webhooks when properties change on your records. This is how you build reactive workflows — something changes in HubSpot, n8n picks it up instantly and does something.
...
A practical example: when a deal stage changes to Closed Won in HubSpot, the webhook fires, n8n picks it up, fetches the full deal and contact record, posts a message in Slack, creates a task in the project management tool, and sends the contract details to your billing system. All of this in under 10 seconds.`,
      ...
      {
        heading: "Building bidirectional syncs",
        body: `The most complex thing you can build with the HubSpot and n8n combination is a bidirectional sync between HubSpot and another system. Data flows both ways: changes in HubSpot update the other system, and changes in the other system update HubSpot.
...
Building this correctly the first time saves hours of debugging later. n8n's execution logs make it easy to spot loops when they happen — you will see the same workflow firing repeatedly in quick succession.`,
  ...
  {
    slug: "gtm-prompts-that-work",
    title: "How to write GTM prompts that actually produce usable output",
    category: "AI",
    readTime: "8 min read",
    date: "May 2026",
    desc: "Most GTM prompts fail for the same reason: they are too vague. Here is the framework I use to write prompts that produce consistent, usable output every time.",
    tools: ["Claude", "GPT-4o", "Clay", "n8n"],
    sections: [
      {
        heading: "Why most GTM prompts fail",
        body: `The most common GTM prompt failure is not a writing problem. It is a specificity problem. Prompts that produce generic output are almost always prompts that ask for something open-ended without telling the model what to do with the result.
...
The shift from vague to specific is the entire skill. Everything else — tone, format, length — is secondary. Get the question right first.`,
      ...
      {
        heading: "Constrain the output before you refine the prompt",
        body: `Constraint is the most powerful lever in prompt writing. The more tightly you define what you want back, the more consistent and usable the output.
...
The test for whether your constraint is tight enough: could you paste the output directly into whatever comes next without editing? If yes, your constraint is working. If no, tighten it.`,
      ...
      {
        heading: "Assign a role before the task",
        body: `Starting your prompt with a role description changes the depth and tone of the output more than most people expect. The model adjusts its reasoning based on the identity you give it.
...
The role works because it activates a more specific set of patterns in the model's response. Be specific about the role. "Expert" is not specific. "Sales development representative who has sent over 10,000 cold emails" is specific. The more real the role sounds, the more useful the output tends to be.`,
      ...
      {
        heading: "Give the model what it cannot infer",
        body: `If you are running a prompt in Clay, the model does not know what your product does, who your ICP is, or why this specific company is on your list. You have to tell it. Prompts that produce generic output are almost always missing this context.
...
In n8n, pass the context as part of the system message and use expressions to inject the row-specific data into the user message. The separation between static context and dynamic data makes your prompts easier to maintain and debug.`,
      ...
      {
        heading: "Debugging prompts that are not working",
        body: `When a prompt is not working, the instinct is to rewrite the whole thing. That is almost always the wrong move. Add one sentence of additional context and see if that fixes it first.
...
When testing in Clay, duplicate three to five rows that represent different types of companies on your list — different sizes, different industries, different data completeness — and run the AI column on those before running the full table. Fix what breaks in the test rows before committing credits to the full enrichment.`,
  ...
  {
    slug: "claude-code-n8n-functions",
    title: "How to use Claude Code to write and debug n8n function nodes",
    category: "Claude Code",
    readTime: "6 min read",
    date: "May 2026",
    desc: "The function node in n8n is where most people get stuck. Claude Code makes writing and fixing the JavaScript significantly less painful once you understand how to give it the right context.",
    tools: ["Claude Code", "n8n", "JavaScript"],
    sections: [
      {
        heading: "Why the function node is where people get stuck",
        body: `The function node in n8n lets you write JavaScript to transform data, run custom logic, or handle anything the built-in nodes cannot do. It is the escape hatch for everything else.
...
Claude Code removes most of this friction. But only if you know how to give it the right context. Without the actual data structure the node is working with, Claude will guess — and it will often guess wrong.`,
      ...
      {
        heading: "The most important thing: show Claude the actual data",
        body: `The most common mistake when asking Claude Code to write a function node is describing what the data looks like rather than showing it. Claude needs the actual JSON structure to write code that correctly references the right fields.
...
This one change — showing the real data instead of describing it — eliminates most of the back-and-forth in function node debugging.`,
      ...
      {
        heading: "How to ask Claude to write a function node",
        body: `The prompt structure that works consistently: paste the input data, describe the transformation you want, specify what the output should look like, and ask for function node code compatible with n8n.
...
If you want Claude to add error handling, ask for it explicitly. By default it will write the happy path. For production workflows, ask it to wrap the main logic in a try-catch and return the original item with an error field set if something fails. This prevents one bad row from crashing the entire workflow.`,
      ...
      {
        heading: "Debugging existing function nodes with Claude",
        body: `When a function node fails, you have three pieces of information: the input data, the current code, and the error message. Give Claude all three and tell it what you expected to happen versus what actually happened.
...
For both of these, Claude will fix the immediate error. Ask it also to scan the rest of the code for the same pattern and fix any other instances. This prevents the same class of error from appearing on a different row later.`,
      ...
      {
        heading: "Compatibility issues and how to handle them",
        body: `n8n's hosted version and some self-hosted versions run older Node.js environments. Modern JavaScript syntax that works fine locally may error in production.
...
The function node is powerful for synchronous data transformation. For anything involving external requests or async logic, the built-in nodes are the better path.`,
  ...
  },
  {
    ...
    title: "How to automate lead routing with webhooks in n8n",
    category: "n8n",
    readTime: "7 min read",
    date: "May 2026",
    desc: "When a new lead comes in, five or six things need to happen at once. Most teams do this manually. Here is how to automate the entire handoff with one webhook workflow.",
    tools: ["n8n", "HubSpot", "Slack", "OpenAI", "Clearbit"],
    sections: [
      {
        heading: "What lead routing actually is",
        body: `Lead routing is the process of deciding what happens to a new lead when it comes in and making sure all of it happens consistently. Who gets notified, which system gets updated, which sequence the lead goes into, which team member owns the follow-up.
...
Automating lead routing with n8n replaces all of this with a single webhook workflow that runs in under 30 seconds for every lead that comes in.`,
      ...
      {
        heading: "Mapping the routing logic before you build",
        body: `Before you open n8n, map out exactly what needs to happen when a new lead comes in. Write it as a sequence of steps, not as a flowchart. Every step becomes a node.
...
If your routing has conditional logic — enterprise leads go to one rep, SMB leads go to another, specific industries get a different sequence — map those branches explicitly before you build. Adding branching logic after the fact to a workflow that was built assuming a single path is painful.`,
      ...
      {
        heading: "Building the webhook entry point",
        body: `Create a new n8n workflow and add a Webhook trigger node. This is your entry point. Every lead that comes in will hit this URL.
...
One practical note: if you are receiving leads from a third-party tool like Typeform, HubSpot forms, or Apollo, check their webhook documentation to understand their payload structure. The field names are often not obvious.`,
      ...
      {
        heading: "Enrichment, classification, and notification",
        body: `After the webhook node, add a HubSpot node set to create or update contact. Map the fields from the incoming payload to the HubSpot contact properties. Use the email address as the unique identifier so existing contacts get updated rather than duplicated.
...
The whole workflow from webhook to Slack notification runs in under 30 seconds. The rep sees a structured brief with everything they need to decide on follow-up. No manual research, no context switching, no leads falling through.`,
      },
      {
        ...
        body: `Every step in this workflow can fail. The enrichment API can time out. HubSpot can return an error. OpenAI can produce malformed JSON. If you do not handle failures, one API error kills the entire routing workflow for that lead.
...
Add a final node that sends an error summary to a dedicated Slack channel when any node fails. This keeps the workflow running for the lead while making sure you know something went wrong. You can review and fix the failure without it silently missing leads.`,
  ...
  {
    slug: "claude-code-sales-ops",
    title: "How to use Claude Code as a sales ops analyst",
    category: "Claude Code",
    readTime: "7 min read",
    date: "May 2026",
    desc: "Claude Code is not just for writing code. Used correctly, it is one of the fastest pipeline analysis and reporting tools available. Here is how I use it for sales ops work every week.",
    tools: ["Claude Code", "HubSpot", "Google Sheets", "Python"],
    sections: [
      {
        heading: "Rethinking what Claude Code is for",
        body: `Most people use Claude Code to write or debug code. That is a fraction of what it can do.
...
For sales ops specifically, this means you can use it to analyse pipeline data, interpret CRM exports, flag deals at risk, build reporting logic, and prepare for pipeline reviews without spending the morning in a spreadsheet. You put the data in. You ask a specific question. You get something actionable back.`,
      ...
      {
        heading: "Analysing pipeline data without writing queries",
        body: `The most common way I use Claude Code for sales ops is pipeline analysis. Pull a CSV export from HubSpot — deals, stages, close dates, deal owners, last activity. Open Claude Code. Paste in the first 30 rows so it can read the column structure and data format.
...
The questions that produce the most useful output are narrow and specific. "Analyse my pipeline" produces nothing useful. "Which deals in the proposal stage have had no activity in the last 14 days and close date within this quarter?" produces a list you can act on immediately.`,
      ...
      {
        heading: "Preparing for pipeline reviews",
        body: `Pipeline reviews require the same analysis every week: which deals moved, which stalled, what is at risk, where is the team spending time. The preparation is repetitive. It should be automated.
...
For trend analysis across multiple weeks, I export the same pipeline data two weeks in a row and paste both snapshots into Claude Code, labelling them by date. I ask it to identify which deals deteriorated, which accelerated, and what the pattern looks like. This kind of comparative analysis is tedious manually and fast with Claude.`,
      ...
      {
        heading: "Building reports and formulas",
        body: `Describing what you want a report to show and getting working formulas back is significantly faster than building them from scratch, especially for anything involving nested logic.
...
The skill is knowing that you can ask for formulas and configurations in natural language and expect working output. Most people do not think to use Claude Code this way because they associate it with code. But formulas and report logic are just code in a different syntax.`,
      ...
      {
        heading: "What Claude Code cannot do in sales ops",
        body: `There are clear limits worth knowing upfront so you do not waste time on things that will not work.
...
export const CATEGORIES = ["All", "Claude Code", "n8n", "Clay", "Outbound", "AI"];

export const categoryColors: Record<string, { color: string; bg: string; border: string }> = {
  "Claude Code": { color: "#c9963b", bg: "rgba(201,150,59,0.1)",  border: "rgba(201,150,59,0.25)" },
  "n8n":         { color: "#9b8afb", bg: "rgba(155,138,251,0.1)", border: "rgba(155,138,251,0.25)" },
  "Clay":        { color: "#4ade80", bg: "rgba(74,222,128,0.1)",  border: "rgba(74,222,128,0.25)" },
  "Outbound":    { color: "#5b9bd5", bg: "rgba(91,155,213,0.1)",  border: "rgba(91,155,213,0.25)" },
  "AI":          { color: "#f87171", bg: "rgba(248,113,113,0.1)", border: "rgba(248,113,113,0.25)" },
};
